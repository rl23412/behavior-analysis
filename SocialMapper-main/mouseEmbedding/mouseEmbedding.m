...
% Calculate lengthtL for individual mouse data  
lengthtL = zeros(32,1);  
for i = 1:32  
    ma1 = allMOUSEL{i};  
    ...
    lengthtL(i) = prctile(sj, 95);  
end
allMOUSES = cell(48,2);  
for i = 1:48  
    xi = i+32;  % This gives indices 33-80  
    load(mouseOrderShort{xi});  % Use xi, not i  
...
allMOUSE_files = cell(length(allMOUSE), 1);  
% Individual mouse files (first 32)  
for i = 1:32  
    allMOUSE_files{i} = mouseOrderShort{i};  
end  
% Social mouse files (next 48)  
for i = 1:48  
    allMOUSE_files{32+i} = mouseOrderShort{32+i};  
end
for j = 1:length(allMOUSE)
    clear ma ma1 m1Dist nnData
    ma1 = allMOUSE{j};
...
allMOUSES_files = cell(48,2);  
for i = 1:48  
    xi = i + 32;  % Social files start after the 32 individual files  
    % Both animals in the social session use the same base filename  
    allMOUSES_files{i,1} = mouseOrderShort{xi};  % Animal 1  
    allMOUSES_files{i,2} = mouseOrderShort{xi};  % Animal 2 (same file)  
end
vecs6 = vecs(:,1:6);
minF = .2; maxF = 5; pcaModes = 10; numModes = pcaModes;
parameters = setRunParameters([]);
...
for rec = 1:length(allMOUSES)
    ...
    save(['mouse/RE_soc/RE_SOC_' num2str(rec) '.mat'],'z1','inCH1','z2','inCH2','filename')
end
...
save('reembeddingInfoLone.mat','xx','LL','vSmooth','medianLength','pThreshold','minRest','obj','numGMM')
%%
...
scM = [chead; chead; chead; cspine; cspine; cspine; cspine; cLF; cLF; ...  
    ...
    cLH; cLH; cLH; cLH; cRH; cRH; cRH; cRH];
WRL = cell(size(EVL));
% Suppress figure creation during watershed processing
fprintf('Processing individual watershed regions (suppressing intermediate plots)...\n');
...
% Restore figure visibility and clean up
...
% Suppress figure creation during social watershed processing
fprintf('Processing social watershed regions (suppressing intermediate plots)...\n');
original_vis_soc = get(0, 'DefaultFigureVisible');
set(0, 'DefaultFigureVisible', 'off');
...
% Restore figure visibility and clean up
set(0, 'DefaultFigureVisible', original_vis_soc);
close all; % Close any figures created during watershed processing
...
%%
% Define social behavioral categories  
...
maxCluster = max(cellfun(@(x) max(x(:)), wrFINE));  
if maxCluster > 118  
    % Extend labelsL with default mapping (e.g., to idle behavior)  
    labelsL(119:maxCluster) = 1; % Map extra clusters to idle  
end
...
allMOUSEL_files = mouseOrderShort(1:32);  % First 32 are individual mouse files  
load('mouseFileOrder.mat', 'mouseOrderShort');  
allMOUSES_files = mouseOrderShort(33:80);  % Social files are typically after individual files  
...
% Create LLC (coarse behavioral map) from LL2 and labelsL  
LLC = zeros(size(LL2));  
for i = 1:128  
    cid = find(LL2==i);  
    LLC(cid) = labelsL(i);  
...
% Create velocity calculations for all mouse recordings  
mouseCV = cell(128,1);  
for i = 1:128  
    pred = allMOUSE{i};  
...
% Create velocity calculations and ordering variables  
allcv = combineCells(mouseCV);  
allwr = combineCells(wrFINE);  

mouseOrderL = []; mouseOrderLTag = [];  
% Extend meancv to handle all cluster IDs  
maxCluster = max(cellfun(@(x) max(x(:)), wrFINE));  
meancv = zeros(maxCluster,1);  
...
for b = 1:maxCluster  
    ...
    if ~isempty(idx)  
        ...
        meancv(b) = 0; % Default value for unused clusters  
    end  
end
for b = 1:14  
    xi = find(labelsL==b);  
...
% Then create mouseOrderS by sorting social behavioral clusters  
...
% For individual mouse files (mouseTagsL)    
mouseTagsL = cell(32,1);    
for i = 1:32    
    load(mouseOrderShort{i});  
    % Extract strain from sdannce.mouse_id (format like 'B1', 'W7')  
    if isfield(sdannce, 'mouseid') && ~isempty(sdannce.mouseid)  
        strain_full = sdannce.mouseid;  
        mouseTagsL{i} = strain_full(1); % Extract just 'B' or 'W'  
    else  
        ...
        if length(parts) >= 2  
            strain_full = parts{2};  
            mouseTagsL{i} = strain_full(1);  
        else  
            mouseTagsL{i} = f(end-1:end);  
        end  
    end  
end    
    
% For social mouse files (mouseTagsS)     
mouseTagsS = cell(48,2);    
for i = 1:48    
    xi = i+32;  
    load(mouseOrderShort{xi});  
      
    % Extract strain from sdannce.mouse_id and sdannce.mouseid_p  
    if isfield(sdannce, 'mouseid') && isfield(sdannce, 'mouseid_p') && ...  
       ~isempty(sdannce.mouseid) && ~isempty(sdannce.mouseid_p)  
          
        strain1_full = sdannce.mouseid;     % First animal (format like 'B1', 'W7')  
        strain2_full = sdannce.mouseid_p;    % Partner animal (format like 'B1', 'W7')  
          
        mouseTagsS{i,1} = strain1_full(1);   % Extract just 'B' or 'W'  
        mouseTagsS{i,2} = strain2_full(1);   % Extract just 'B' or 'W'  
        ...
        if length(parts) >= 2  
            strain1_full = parts{2};  
            mouseTagsS{i,1} = strain1_full(1);  
              
            if length(parts) >= 3 && ~isempty(regexp(parts{3}, '[WB]\d+', 'once'))  
                strain2_full = parts{3};  
                mouseTagsS{i,2} = strain2_full(1);  
            else  
                mouseTagsS{i,2} = strain1_full(1);  
            end  
        else  
            mouseTagsS{i,1} = 'UNKNOWN';  
            mouseTagsS{i,2} = 'UNKNOWN';  
        end  
    end  
end
...
socDist = zeros(1,maxClusterS_actual);
...
%% Dynamic Dimension Handling - Handle cluster count mismatches

% Detect actual maximum cluster numbers from the data
maxClusterL_actual = 0;  % Individual clusters
maxClusterS_actual = max(max(LL2soc));  % Social clusters (should be 109)

% Find actual max individual cluster from wrFINE
for i = 1:length(wrFINE)
    if ~isempty(wrFINE{i})
        maxClusterL_actual = max(maxClusterL_actual, max(wrFINE{i}));
    end
end

fprintf('Detected %d individual behavioral clusters (expected 118)\n', maxClusterL_actual);
fprintf('Detected %d social behavioral clusters (expected 133)\n', maxClusterS_actual);

% Extend labelsL if needed to handle all clusters
if maxClusterL_actual > 118
    fprintf('Extending labelsL from 118 to %d clusters\n', maxClusterL_actual);
    labelsL_extended = zeros(1, maxClusterL_actual);
    labelsL_extended(1:118) = labelsL;
    % Assign unclassified clusters to a default category (15)
    labelsL_extended(119:end) = 15;
    labelsL = labelsL_extended;
end

% Create filtered versions of social indices
% Since mouseOrderS contains 133 elements but data only has maxClusterS_actual columns
if exist('mouseOrderS', 'var') && ~isempty(mouseOrderS)
    validSocialIndices = mouseOrderS(mouseOrderS <= maxClusterS_actual);
    nValidSocial = length(validSocialIndices);
else
    validSocialIndices = 1:maxClusterS_actual;
    nValidSocial = maxClusterS_actual;
end

% Update socDist array to handle actual cluster count
if exist('socDist', 'var') && length(socDist) == 133 && maxClusterS_actual < 133
    % Adjust socDist for clusters that don't exist
    socDist_actual = socDist(1:maxClusterS_actual);
else
    socDist_actual = socDist;
end

% Create dimension variables for consistent use throughout
nIndividualClusters = size(loneUsage, 2);  % Actual individual usage columns
nSocialClusters = maxClusterS_actual;      % Actual social clusters (109)

fprintf('Individual behavior columns: %d\n', nIndividualClusters);
fprintf('Social behavior columns: %d\n', nSocialClusters);

%% ==== IMPORTANT: Dynamic Dimension Variables Reference ====
% Use these variables instead of hardcoded values throughout the code:
%
% Individual (Lone) Behavior Dimensions:
%   - maxClusterL_actual    : Maximum cluster ID found in wrFINE
%   - nIndividualClusters   : Number of columns in loneUsage 
%   - length(labelsL)       : Extended label array length
%   - maxLL2                : Maximum value in LL2 watershed regions
%
% Social Behavior Dimensions:  
%   - maxClusterS_actual    : Maximum cluster ID in LL2soc (typically 109, not 133)
%   - nSocialClusters       : Same as maxClusterS_actual
%   - validSocialIndices    : Filtered mouseOrderS indices that exist in data
%   - nValidSocial          : Length of validSocialIndices
%
% When visualizing or analyzing:
%   - Use size(BBsl, 2) instead of 118 for individual behavior columns
%   - Use size(BBs, 2) instead of 133 for social behavior columns  
%   - Use validSocialIndices when accessing social data with mouseOrderS
%   - Check bounds before accessing labelsL or labelsS arrays
% ============================================================

%% Variable Dependency Check for Visualization
% Check if critical variables exist for density maps and plots

missing_vars = {};

% Individual behavior density and category variables
if ~exist('D', 'var')
    missing_vars{end+1} = 'D (individual density map - created around line 712)';
end
if ~exist('LLC', 'var')
    missing_vars{end+1} = 'LLC (individual category map - created around line 708)';
end
if ~exist('llbwb', 'var')
    missing_vars{end+1} = 'llbwb (individual watershed boundaries - created around line 720)';
end

% Social behavior density and category variables  
if ~exist('dsoc', 'var')
    missing_vars{end+1} = 'dsoc (social density map - created at line 939)';
end
if ~exist('llbwbsoc', 'var')
    missing_vars{end+1} = 'llbwbsoc (social watershed boundaries - created around line 950)';
end

% Color maps
if ~exist('colorsCoarse', 'var')
    missing_vars{end+1} = 'colorsCoarse (behavioral category colors - defined at line 1322)';
end
if ~exist('cmap1', 'var')
    missing_vars{end+1} = 'cmap1 (density plot colormap - defined earlier)';
end
if ~exist('cmapSOC', 'var')
    missing_vars{end+1} = 'cmapSOC (social category colormap - defined earlier)';
end

% Core data matrices
if ~exist('loneUsage', 'var')
    missing_vars{end+1} = 'loneUsage (individual behavior usage matrix)';
end
if ~exist('socUsage1', 'var') || ~exist('socUsage2', 'var')
    missing_vars{end+1} = 'socUsage1/socUsage2 (social behavior usage matrices)';
end

% Display warning if variables are missing
if ~isempty(missing_vars)
    fprintf('WARNING: %d critical variables are missing for visualization:\n', length(missing_vars));
    for i = 1:length(missing_vars)
        fprintf('  - %s\n', missing_vars{i});
    end
    fprintf('\nTo fix this, you need to run earlier sections of the code that create these variables.\n');
    fprintf('Recommended: Run from line 700 (after embedding) instead of line 1000.\n\n');
    
    % Try to load from saved file if available
    try
        fprintf('Attempting to load missing variables from saved file...\n');
        load('mouse_embedding_info.mat');
        fprintf('Successfully loaded some variables from mouse_embedding_info.mat\n');
    catch
        fprintf('Could not load from mouse_embedding_info.mat\n');
    end
else
    fprintf('✓ All critical variables present for visualization\n');
end

%% Essential Colormap Definitions (in case missing from earlier sections)
if ~exist('colorsCoarse', 'var')
    fprintf('Creating missing colorsCoarse colormap...\n');
...
if ~exist('cmap1', 'var')
    fprintf('Creating missing cmap1 colormap...\n');
    cmap1 = parula(256);  % Default density colormap
end

if ~exist('cmapSOC', 'var')
    fprintf('Creating missing cmapSOC social colormap...\n');
...
if ~exist('cmapdiff', 'var')
    fprintf('Creating missing cmapdiff difference colormap...\n');
...
%%
cmapLL = [1 1 1;           % 0 - background/unlabeled (white)  
    0 .16 1;               % 1 - idle (blue)  
    0 .3833 1;             % 2 - slow (light blue)  
    0 .7222 1;             % 3 - head (cyan)  
    .1333 1 .8667;         % 4 - groom (green-cyan)  
...
% Determine actual max cluster from wrFINE
maxClusterActual = max(allwr);
meancv = zeros(maxClusterActual,1);

for b = 1:maxClusterActual
    idx = find(allwr==b);
    if ~isempty(idx)
        mdc = median(allcv(idx),'omitnan');
        meancv(b) = mdc;
    else
        meancv(b) = 0; % Default for empty clusters
    end
end
...
nColsDisplay = size(mouseBLd1, 2);  % Use actual columns from the data
imagesc(log([mouseBLd1; inf(1,nColsDisplay); mouseBLd2; inf(3,nColsDisplay); ...
    mouseWLd1; inf(1,nColsDisplay); mouseWLd2]));
colormap(gray);
maxS = max(max(LL2soc));  % This should be 109, not 133  

% Filter mouseOrderS to only include valid indices  
validIndices = mouseOrderS(mouseOrderS <= size(socUsage1, 2));  
nCols = length(validIndices);  
  
SS1_spaced = [socUsage1(1:12, validIndices); inf(1,nCols);   
              socUsage1(13:28, validIndices); inf(1,nCols);   
              socUsage1(29:48, validIndices)];

nColsSL1 = size(SL1(:,mouseOrderL), 2);  % Get actual columns after ordering
SL1_spaced = [SL1(1:12,mouseOrderL); inf(1,nColsSL1); SL1(13:28,mouseOrderL); inf(1,nColsSL1); SL1(29:48,mouseOrderL)];
imagesc(log(SL1_spaced)); colormap(gray)

validIndices = mouseOrderS(mouseOrderS <= size(socUsage2, 2));  
nCols = length(validIndices);  
  
SS2_spaced = [socUsage2(1:12, validIndices); inf(1,nCols);   
              socUsage2(13:28, validIndices); inf(1,nCols);   
              socUsage2(29:48, validIndices)];  
imagesc(log(SS2_spaced)); colormap(gray);


nCols = size(socUsage1, 2);  % Use actual number of columns (109)  
 colormap(gray)
...
% Close any remaining figures before creating main plots
close all;

% Individual behavior density maps (Named Figures)
if exist('LLC', 'var') && exist('llbwb', 'var')
    fig1 = figure('Name', 'Individual Behavior Categories', 'NumberTitle', 'off');
    imagesc(LLC); hold on; colormap(colorsCoarse); caxis([0 14])
    hold on; scatter(llbwb(:,2),llbwb(:,1),'.','k'); axis equal; 
    title('Individual Behavior Categories'); axis off;
    
    if exist('D', 'var')
        fig2 = figure('Name', 'Individual Behavior Density', 'NumberTitle', 'off');
        imagesc(D); colormap(flipud(gray)); caxis([0 6e-4]);
        hold on; scatter(llbwb(:,2),llbwb(:,1),'.','k'); axis equal; 
        title('Individual Behavior Density'); axis off;
    else
        fprintf('Warning: D (density map) not available\n');
    end
else
    fprintf('Warning: LLC or llbwb not available for individual behavior maps\n');
end
...
% Create labelsS with actual social cluster count
...
% Filter out indices that exceed actual cluster count
validIndicesS = 1:maxClusterS_actual;
...
% Social behavior density maps (Named Figures)
if exist('dsoc', 'var') && exist('llbwbsoc', 'var')
    fig3 = figure('Name', 'Social Behavior Density', 'NumberTitle', 'off');
    imagesc(dsoc); axis equal off; colormap(cmap1); caxis([0 3e-4]);
    hold on; scatter(llbwbsoc(:,2),llbwbsoc(:,1),1,'k','.');
    title('Social Behavior Density'); axis off;
    ...
    if exist('LLCsoc', 'var')
        ...
        title('Social Behavior Categories'); axis off;
        set(gcf,'Position',[653 242 868 759]);
    else
        fprintf('Warning: LLCsoc (social category map) not available\n');
    end
else
    fprintf('Warning: dsoc or llbwbsoc not available for social behavior maps\n');
end
...
for b = 1:7
    xi = find(labelsS==b);
    % Filter out indices that exceed the actual cluster count
    xi_valid = xi(xi <= maxClusterS_actual);
    if ~isempty(xi_valid)
        dxi = socDist(xi_valid);
        [dsorted, dsxi] = sort(dxi,'descend');
        sortedxi = xi_valid(dsxi)';
        mouseOrderS = [mouseOrderS; sortedxi];
        mouseOrderSTag = [mouseOrderSTag; b*ones(size(sortedxi))];
    end
end
...
% Use filtered mouseOrderS for bar plot
validOrderS = mouseOrderS(mouseOrderS <= maxClusterS_actual);
b = bar(socDist(validOrderS));
b.FaceColor = 'flat';
for i = 1:length(validOrderS)
    if i <= length(mouseOrderSTag)
...
% Create socUsageAll with filtered indices  
socUsageAll = [socUsage1(:,validIndices); socUsage2(:,validIndices)];
...
hup = hist(up,1:7); hdown = hist(down,1:7);
...
% Statistical Difference Plots (Figure 20-21 to avoid conflicts)
cmapdiff = [  
    0 0 1;      % Blue for negative differences  
    0.5 0.5 1;  % Light blue  
    1 1 1;      % White for no difference  
    1 0.5 0.5;  % Light red  
    1 0 0       % Red for positive differences  
];
nValidCols = size(ssig, 2);  % Should be 109  
  
figure(20); % Social behavior differences
diffss = [diffBB_WW; diffBW_BB; diffWB_WW];  
imagesc(diffss); colormap(cmapdiff); caxis([-.05 .05]); hold on;  
for g = 1:3  
    for b = 1:nValidCols  % Use actual size instead of hardcoded 133  
...
title('Social Behavior Differences Between Groups');


figure(21); % Individual behavior differences 
diffsl = [diffBB_WWsl; diffBW_BBsl; diffWB_WWsl];
imagesc(diffsl); colormap(cmapdiff); caxis([-.05 .05]); hold on;
for g = 1:3
    for b = 1:size(diffsl, 2)  % Use actual number of columns
...
set(gcf,'Position',[45 835 1332 55]); axis off;
title('Individual Behavior Differences Between Groups');
...
%%  mouse social
% PCA for lone behavior (individual strain comparison)
...
axis equal; title('PCA Individual Mouse Strains (B vs W)');
...
scatter3(SCORE(:,1),SCORE(:,2),SCORE(:,3),200,colorSoc,'filled'); hold on; 
axis equal; title('PCA Social Individual Behavior');
...
scatter3(SCORES(:,1),SCORES(:,2),SCORES(:,3),200,colorSoc,'filled'); hold on; 
axis equal; title('PCA Social Interactive Behavior'); 
...
% Use dynamic column counts instead of hardcoded dimensions
nColsL = size(BBsl, 2);  % Actual individual behavior columns
nColsS = size(BBs, 2);   % Actual social behavior columns

% Individual behavior usage heatmaps
fig12 = figure('Name', 'Individual Behavior Usage Heatmap', 'NumberTitle', 'off');
imagesc(log([BBsl; ones(2,nColsL); BWsl; ones(2,nColsL); WWsl; ones(2,nColsL); WBsl])); 
colormap(gray); caxis([-9 -2]); axis off;
set(gcf,'Position',[278 815 669 172]);
title('Individual Behavior Usage by Group');

% Social behavior usage heatmaps
fig13 = figure('Name', 'Social Behavior Usage Heatmap', 'NumberTitle', 'off');
imagesc(log([BBs; ones(2,nColsS); BWs; ones(2,nColsS); WWs; ones(2,nColsS); WBs])); 
colormap(gray); caxis([-9 -2]); axis off;
set(gcf,'Position',[278 815 669 172]);
title('Social Behavior Usage by Group');

% Mean calculations already done above, remove duplicate

% Calculate mean-subtracted matrices for visualization
mBB = BBsl-meanBBsl; 
mWW = WWsl-meanBBsl; 
mWB = WBsl-meanBBsl; 
mBW = BWsl-meanBBsl; 

% Combined individual behavior difference plots
fig14 = figure('Name', 'Individual Behavior Differences', 'NumberTitle', 'off');
imagesc([mBB; zeros(2,nColsL); mBW; zeros(2,nColsL); mWW; zeros(2,nColsL); mWB]); 
colormap(cmapdiff); caxis([-.1 .1]);
set(gcf,'Position',[278 815 669 172]); axis off;
title('Individual Behavior Differences from BB Mean');

% Combined social behavior difference plots  
...
colormap(cmapdiff); caxis([-.1 .1]);
set(gcf,'Position',[278 815 669 172]); axis off;
title('Social Behavior Differences from Group Means');




% Mean behavior profiles for each group  
...
fprintf('Mouse behavior analysis completed successfully!\n');
fprintf('Generated named figures (no numerical interference):\n');
fprintf('- Individual Behavior Categories & Density\n');
fprintf('- Social Behavior Categories & Density\n'); 
fprintf('- PCA Individual Mouse Strains\n');
fprintf('- PCA Social Individual Behavior\n');
fprintf('- PCA Social Interactive Behavior\n');
fprintf('- Individual & Social Behavior Usage Heatmaps\n');
fprintf('- Individual & Social Behavior Differences\n');
fprintf('- Mean Individual & Social Behavior Profiles\n');

%% =================================================================
%% SUMMARY OF EXPECTED FIGURES
%% =================================================================
fprintf('\n=== FIGURE SUMMARY ===\n');
fprintf('BEHAVIORAL DENSITY MAPS (Main Results):\n');
fprintf('  Figure 1: Individual Behavior Categories (colored regions)\n');
fprintf('  Figure 2: Individual Behavior Density (grayscale density)\n');
fprintf('  Figure 3: Social Behavior Density (colored density)\n');
fprintf('  Figure 4: Social Behavior Categories (colored regions)\n\n');

fprintf('STATISTICAL ANALYSIS:\n');
fprintf('  Figures 9-11: PCA scatter plots (strain comparisons)\n');
fprintf('  Figures 12-13: Usage heatmaps (gray scale)\n');  
fprintf('  Figures 14-15: Difference heatmaps (blue-red)\n');
fprintf('  Figures 16-17: Mean behavior profiles\n');
fprintf('  Figures 20-21: Statistical difference plots\n\n');

fprintf('If Figures 1-4 are missing, check that these variables exist:\n');
fprintf('  - D, LLC, llbwb (for individual maps)\n');
fprintf('  - dsoc, LLCsoc, llbwbsoc (for social maps)\n');
fprintf('  Run from line 700+ to ensure all variables are created.\n');
fprintf('=========================\n');
