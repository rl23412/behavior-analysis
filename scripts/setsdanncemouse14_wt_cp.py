...
"""
Three-camera WT and CP(full) Anipose to s-DANNCE conversion (mouse14 skeleton).

Builds s-DANNCE-ready directories similar to the control-set workflow but
exports only the 14-joint mouse skeleton for both WT and CP(full) datasets.
"""
...
# Mapping from the mouse14 skeleton to DLC joints.
joint_mapping = {
    "Snout": "nose",
    "EarL": "left_earend",
    "EarR": "right_earend",
...
MOUSE14_LEFT_KEYPOINTS = [1, 6, 7, 10, 11]
MOUSE14_RIGHT_KEYPOINTS = [2, 8, 9, 12, 13]
...
DATASET_CONFIGS = {
    'wt': {
        ...
        'output_base_dir': '/work/rl349/dannce_mouse14/wt_mouse14'
    },
    'cpfull': {
        ...
        'output_base_dir': '/work/rl349/dannce_mouse14/cpfull_mouse14'
    }
}
...
def load_anipose_3d_labels(csv_path):
    """Load Anipose 3D tracking results from CSV and map to the mouse14 skeleton."""
    # Read the CSV file
    ...
    return {
        'data_3d': data_3d,
        'frames': frames,
        'keypoint_names': TARGET_MOUSE14_JOINTS,
        'scores': scores,
        'ncams': ncams
    }
...
def generate_sdannce_config(video_mapping, output_base_dir):
    ...
    config = {
        ...
        # Architecture tuned for mouse14/WT recordings
        'expval': True,
        'net_type': 'dannce',
        'n_channels_in': 3,
        'n_channels_out': len(TARGET_MOUSE14_JOINTS),
        'new_n_channels_out': len(TARGET_MOUSE14_JOINTS),
        'n_views': len(CAMERA_ORDER),
        ...
        # Loss configuration - updated to use mouse14 profile
        ...
        # Data augmentation - use mouse14 left/right groupings
        'medfilt_window': 30,
        'rand_view_replace': True,
        'n_rand_views': len(CAMERA_ORDER),
        'mirror_augmentation': True,
        'left_keypoints': MOUSE14_LEFT_KEYPOINTS,
        'right_keypoints': MOUSE14_RIGHT_KEYPOINTS,
        'augment_hue': False,
    ...
    print(f"  - Configured for mouse14 skeleton ({len(TARGET_MOUSE14_JOINTS)} keypoints)")
...
def convert_dataset(dataset_key, dataset_cfg):
    ...
    print(f"=== {description} to s-DANNCE Conversion (mouse14, 3 cameras) ===\n")
    ...
    print(f"Note: Using mouse14 skeleton with {len(TARGET_MOUSE14_JOINTS)} keypoints and three cameras")
